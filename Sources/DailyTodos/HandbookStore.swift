...
// 性能优化:将 handbookItems 从 TodoStore 拆分到独立的 HandbookStore。
// 原实现中,todos 与 handbookItems 共用一个 ObservableObject,
// 任何一边的 @Published 变化都会让所有观察者(todos 列表 + handbook 侧栏)的 body 失效。
// 拆分后,toggle 待办不再触发 HandbookSidebarView 重算,编辑手记也不再触发 TodoListView 重算。
@MainActor
final class HandbookStore: ObservableObject {
    ...
    func prefetchHandbookItemsAfterStartup() {
        // 性能优化:启动时立即调度(原 350ms 延迟会造成切换白屏)
        scheduleLoadHandbookItemsIfNeeded(after: nil)
    }
